<?php

namespace app\event;

use Carbon\Carbon;
use GuzzleHttp\Client;
use QL\QueryList;
use support\Redis;

class Onelink
{
    public const url = 'https://1link.fun/blog';
    const userAgent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/109.0.0.0 Safari/537.36';
    public const alias = '1link-weekly';


    public function update()
    {
        try {
            $client = new Client([
                //允许重定向
                'allow_redirects' => true,
            ]);

            $html = $client->get(self::url, [
                'headers' => [
                    //模拟浏览器请求
                    'user-agent' => self::userAgent,
                ]
            ])->getBody()->getContents();

            $ql = new QueryList();
            //解析html
            $table = $ql->html($html)->find('.blog-posts');
            $list = $table->find('li')->map(function ($row) {
                $a = $row->find('a');
                // 过滤掉不包含科技周刊的记录
                if (strpos($a->text(), '科技周刊') === false) {
                    return null;
                }
                return [
                    'title'    => $a->text(),
                    'url'      => $a->attr('href'),
                    'subtitle' => 0
                ];
            })->take(10)->filter()->values()->toArray();
            $list = json_encode($list, JSON_THROW_ON_ERROR | JSON_UNESCAPED_UNICODE);
            Redis::set(self::alias, $list);
            Redis::set(self::alias . '-time', Carbon::now()->toDateTimeString());
            dump(date('Y-m-d H:i:s') . '更新' . self::alias . '成功');
        } catch (\Throwable $e) {
            dump('更新' . self::alias . '异常：' . $e->getMessage());
        }
    }
}